...
#            pool.join()
    ...
    def load_output_format_file(self, _output_format_file):
        ...
        for _line in _fid:
            ...
            if len(_words) == 6: #polar coordinates
            ...
            elif len(_words) == 4: # polar coordinates for submission foramt, it does not have track count and distance
                _output_dict[_frame_ind].append([int(_words[1]), 0, float(_words[2]), float(_words[3])])  # the second is dummy track count
        _fid.close()
        return _output_dict
